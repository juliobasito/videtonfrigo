...
Class Recette
{
	...
	public static function addRecette($nomRecette,$complexite,$note,$temps,$nbPersonne,$description)
	{
		include('bdd.php');
		$sql = $db->prepare('INSERT INTO recette(NomRecette, Complexite, Note, Temps, nbPersonne, description) VALUES (:nomRecette, :complexite, :note, :temps, :nbPersonne, :description)');
		$flag = array('nomRecette' => $nomRecette, 'complexite' => $complexite, 'note'=>$note, 'temps'=>$temps, 'nbPersonne'=>$nbPersonne, 'description'=>$description);
		$sql->execute($flag);	
	}
	public static function addRecette_ingr($ingredientId,$recetteId,$quantite)
	{
		include('bdd.php');
		$sql = $db->prepare('INSERT INTO `recette_ingr`(`IngredientId`, `RecetteId`, `quantite`) VALUES (:ingredientId,:recetteId,:quantite)');
		$flag = array(
	...
	public static function delRecette($recetteId)
	{
		include('bdd.php');
		$sql = $db->prepare('DELETE FROM recette WHERE RecetteId = :recetteId');
		$flag = array('recetteId' => $recetteId);
		$sql->execute($flag);	
	}
}
